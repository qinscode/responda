...
const generateForecastData = (days: number = 7, lastValue: number = 2) => {
	...
	for (let index = 1; index <= days; index++) {
		...
		const value = Math.max(
			0,
			Math.min(6, lastValue + trend + (Math.random() - 0.5) * uncertainty)
		);
...
mockBushfireTimeSeriesAnalysis.anomalies = generateAnomalies(
	mockBushfireTimeSeriesAnalysis.data
);
...
mockFloodTimeSeriesAnalysis.anomalies = generateAnomalies(
	mockFloodTimeSeriesAnalysis.data
);
...
export const mockAdvancedForecasts: Array<AdvancedForecast> =
	getAllMockRegions()
		.slice(0, 5)
		.flatMap((region) => {
			const forecasts = [];
			for (let index = 1; index <= 7; index++) {
				...
				const bushfireLevel: BushfireRating = [
					"no-rating",
					"low-moderate",
					"high",
					"very-high",
					"severe",
				][Math.floor(Math.random() * 5)] as BushfireRating;

				const floodLevel: FloodRating = [
					"no-warning",
					"minor",
					"moderate",
					"major",
				][Math.floor(Math.random() * 4)] as FloodRating;
				...
				forecasts.push({
					...
					modelUsed:
						mockPredictionModels[
							Math.floor(Math.random() * mockPredictionModels.length)
						]!.id,
					uncertaintyRange: {
						min: Math.random() * 2,
						max: 4 + Math.random() * 2,
...
export const mockPredictiveAlerts: Array<PredictiveAlert> = [
	{
		...
		description:
			"Weather patterns indicate high bushfire risk developing over the next 3 days due to rising temperatures and low humidity.",
		probability: 0.78,
	...
	},
	{
		...
		description:
			"Rainfall forecasts suggest possible minor to moderate flooding in low-lying areas within 24-48 hours.",
		probability: 0.65,
...
export const mockRegionalRiskProfiles: Array<RegionalRiskProfile> =
	getAllMockRegions().map((region) => {
...
export const mockAnalyticsDashboardData: AnalyticsDashboardData = {
	summary: {
		...
		highRiskRegions: mockRegionalRiskProfiles.filter(
			(profile) => profile.overallRiskScore > 70
		).length,
		averageRiskScore:
			mockRegionalRiskProfiles.reduce(
				(sum, profile) => sum + profile.overallRiskScore,
				0
			) / mockRegionalRiskProfiles.length,
